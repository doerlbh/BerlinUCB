...
function [new_x, new_y] = K_nonstationary_test(x,y,window)
...
% [x_t, y_t, z_t] = trishuffle([x_first_half;x_second_half],[y_first_half;y_second_half],[z_first_half;z_second_half]);
% disp(['--> finish generating nonstationary dist for window ' num2str(n)]);
% disp(['-> in summary for window :' num2str(n)]);
% tabulate(new_z)
